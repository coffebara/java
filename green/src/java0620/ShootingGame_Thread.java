...
class Boom extends GraphicObject {
...
class MyPanel extends JPanel implements KeyListener {
	...
	public MyPanel() {
		...
		class MyThread extends Thread {
			public void run() {
				while (true) {
					...
					spaceship.update();
					repaint();
					if (Math.abs(missile.x - enemy.x) <80 && Math.abs(missile.y - enemy.y) <80 )  {
		...
		class MyBoomThread extends Thread {
			public void run() {
				while (true) {
					boom.update();
					repaint();
					if (Math.abs(missile.x - enemy.x) <80 && Math.abs(missile.y - enemy.y) <80 ) {
						...
						boom.boom(enemy.x - 10, enemy.y - 65);
						repaint();
						try {
							Thread.sleep(100);
						} catch (InterruptedException e) {
		...
		class MyMissileThread extends Thread {
			public void run() {
				while (true) {
					missile.update();
					repaint();
		...
		Thread t1 = new MyBoomThread();
		Thread t2 = new MyMissileThread();
		
//		t1.setPriority(10);  // MyBoomThread 우선순위 설정
//		t2.setPriority(8);   // MyMissileThread 우선순위 설정
//		t.setPriority(9);   // MyThread 우선순위 설정
