...
// 지정된 구성 요소의 내용을 표시하는 JScrollPane을 만듭니다
...
// 배열의 두 번째 인덱스가 "exit"이면
					if(str[1].equals("eixt")) {
// 해당 메세지를 모든 유저에게 전송하고
						broadCasting(message);
// 스레드를 정지한다
						isStop = true;
// 아니면 메세지를 모든 유저에게 전송한다
					} else {
						broadCasting(message);
					}
				}
// 리스트에서 이 유저의 스레드를 삭제하고
				list.remove(this);
// 해당 유저 ip주소와 메세지를 여러줄 텍스트 영역에 추가한다				
				ta.append(socket.getInetAddress() + "IP주소의 사용자께서 종료하셨습니다.\n");
// 한줄 텍스트 영역에 현재 list 크기로 접속 중인 유저 수를 셋팅하낟
