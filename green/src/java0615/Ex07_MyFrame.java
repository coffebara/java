...
class MyPanel extends JPanel implements ActionListener {
	// 상태를 기록하고 처리 흐름을 제어하기 위한 boolean타입 변수
	boolean flag = false;
	...
	public MyPanel() {
		...
		// 버ㅓ튼이 눌렸을 때 액션리스너가 실행된다.
		// paintComponent 메서드로 이동!
		b.addActionListener(this);
		// 버튼 객체를 출력하는데 남쪽 위치하도록.
		add(b, BorderLayout.SOUTH);
	}

	// JPanel을 상속받고 있기에
	// 항상paintComponent 활용할 때는 JPanel을 활용하자!
	@Override
	protected void paintComponent(Graphics g) {
		// JPanel이나 JLabel처럼 그래픽 컴포넌트인 경우,
		// paintComponent 메서드를 재정의 할 때 내부에 호출되면 좋은 문장이다.
		super.paintComponent(g);
		g.setColor(Color.BLACK);
		// drawOval: 원을 그린다. g.drawOval(x,y, width, height)
		// 좌측을 기준으로 x, y는 0,0이라 생각하고 그 위치에 그리는것
...
public class Ex07_MyFrame extends JFrame {
	public Ex07_MyFrame() {
		// Mypanel 클래스를 추가한다.
		add(new MyPanel());
