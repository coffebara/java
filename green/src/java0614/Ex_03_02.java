...
//버튼생성, 배경색상과 배치관리자 즉)layout 순서
//버튼 만드는 방법 2가지
public class Ex_03_02 extends JFrame{
	public Ex_03_02() {
//이름과 JFrame 창 닫기 시 프로그램이 종료되는 설정
		setTitle("ContetnPane과 JFrame");
...
//JButton 객체는 contetntPane에 추가될 자식 요소가 됩니다.
...
//컨테이너와 컴포넌트의 포함 관계
//(1) 최상위 컨테이너 : 다른 컨테이너에 속하지 않고 독립적으로 출력이 가능한 컨테이너
//						(JFrame, JApplet, JDialog, JWindow)
//(2) 컨테이너 : 다른 컨테이너에 포함될 수 있고, 컴포넌트를 포함할 수 있다(JPanel 등)
//(3) 컴포넌트 : 컨테이너에 포함되어야 화면에 출력 가능(JButton, JLabel, JTextField)

//이벤트 리스너란?
//이벤트를 처리하는 코드, 클래스로 작성(ex. 엿듣고 있는 것)
//이벤트 처리를 위해서는 이벤트 발생시킬 컴포넌트에이벤트 리스너를 연결해야한다. 
//JDK에서 이벤트리스너 작성을 위한 인터페이스(interface) 제공
//-개발자가 리스너 인터페이스의 추상 메서드 구현, 이벤트가 발생하면
//자바 플랫폼은 리스너 인터페이스의 추상 메서드 호출
...
//JFrame의 컨테이너들의 속성(메서드)을 여러개 추가할 수 있다.

//ActionListener
//버튼을 클릭하거나 메뉴를 누르거나 텍스트 필드 등에서 Enter를 눌렀을 때
//발생하는이벤트 Listener .ActionListener는 인터페이스로 처리한다.
//즉) actionPerFormed(ActionEvent e) 액션리스너 이벤트가 발생했을 때
//실질적으로 실현되는 메서드로 꼭 필요한 것 !!
...
//프레임 종료버튼(X)이 클릭됬을 때의 상황
//프레임을 종료하여 프렘 윈도우가 닫힘
//- 프레임이 화ㅏ면에서 보이지 않게 되고 응용 프로그램이 사라짐
//- 프레임이 보이지 않게되지만 응용프로그램이 종료한 것이 아님
//- 키보드나 마우스 입력을 받지 못함
