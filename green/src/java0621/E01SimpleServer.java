...
//클라이언트에서 보낸 데이터를 받아 파일을 만들어 저장한다
public class E01SimpleServer {
	public static void main(String[] args) {
		...
		try {
//플랫폼의 기본 문자 집합을 사용하여 파일 이름이 지정된 FileWriter를구성합니다			
//out.txt라는 파일에 데이터를 입력하는 라이터 (파일 없으면 만듦)
			FileWriter writer = new FileWriter("out.txt");
//포트 5435인 서버 소켓 생성
			serverSocket = new ServerSocket(5435);
			System.out.println("서버 실행 중");
//클라이언트의 연결 요청시 연결된 소켓  획득
			s1 = serverSocket.accept();
//소켓에 대한 입력 스트림 획득
			is = s1.getInputStream();
// 획득한 입력 스트림에 문자 읽을 시 성능 향상을 위해 리더와 버퍼 연결
			br_in = new BufferedReader(new InputStreamReader(is));
// writer 파일에 문자를 입력하기 위한 프린트라이터 생성
