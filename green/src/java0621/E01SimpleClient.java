...
//파일에서 읽어온 데이터를 저장할 변수
			String data =null;
//내 ip에 포트 5434인 소켓을 생성하며 연결
			Socket s1 = new Socket("localhost", 5435);
//소켓에 대한 출력 스트림 획득
			os = s1.getOutputStream();
// 플랫폼의 기본 문자 집합을 사용하여 읽을 파일의 이름이 지정된 새 FileReader를 만듭니다.
//write.txt 라는 파일에 연결해 데이터를  읽어올 리더 생성
			br_out = new BufferedReader(new FileReader("writer.txt"));
//획득한 출력 스트림에 문자 입력 시 성능향상을 위해 라이터와 버퍼 연결
			bw = new BufferedWriter(new OutputStreamWriter(os));
// 버퍼에 담긴 문자를 쓰기 위해 프린트라이터 연결
			pw = new PrintWriter(bw, true);
// write.txt에서 문자열을 한 줄씩 읽어와 data에 저장
//읽어올 데이터가 없으면 반복종료
