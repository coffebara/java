package java0615A;
//현재 동작중인스레드 정보
//스레드란프로세스가 만들어낸 임의의 데이터 처리공간 정도
public class E02ThreadMainEx {
	public static void main(String[] args) {
		long id = Thread.currentThread().getId();
//메서드를 호출하여 현재 스레드의ID를 가져옵니다.
		String name= Thread.currentThread().getName();
		int priority = Thread.currentThread().getPriority();
//메서드를 호출하여 현재 스레드의 우선 순위를 가져옵니다.
...
}	//Runnable 상태 : 스레드를 실행하기 위한 준비 상태 (준비)
	//Running 상태 : 스레드 스케쥴러가 선택된 스레드를 실행하는 상태 (실행)
	//Blocked 상태 : 스레드가 작업을 오나수하지 못하고 잠시 작업을 멈추는 상태(대기)
// new -> Runnable(준비) -> Running(실행) -> Dead
//1. Start() 호출 2. 스레드가 실행 준비 3. 스케쥴러에 의해 스레드가 선택 4. 실행
//5. 스레드가 완료하지 못하고 대기상태로 전환(sleep(), yield(), join() 호출)
//6. 대기상태에서 다시 준비상태로 전환
//sleep() 현재 실행중인 스레드의 실행을 지정한 시간만큼 중지 시킨다.
//yield() 특징
//현재 실행하는 스레드가 다른 스레드를 실행하도록 실행을 중지한다. 
//호출방법Thread.yield();
//실행 중인 스레드가 호출된다.

//스레드 동기화(synchornized)
//멀티스레드 프로세스의 경우여러 쓰레드가 같은 프로세스 내의 자원을 공유해서 작업하기 때문에
//서로의 작업에 영향을 주게 된다.
//따라서 스레드 동기화는 한번에 한 개의 스레드만 객체에 접근할 수 있도록
//객체에 락(lock)을 걸어서 데이터의 일관성을 유지하는 것이다.
//공유 데이터를 사용하는 코드 영역을 임계영역으로 지정해놓고,
//공유 데이터를(객체)가 가지고 있는 lock을 획득한 단 하나의 쓰레드만
