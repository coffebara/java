...
/*  String saveDirectory = "C:\\Uploads"; */
 ...
 try {
	 // 1. MultipartRequest 객체 생성 (request객체, 저장할 파일 경로, 파일 크기, 인코딩 방식)
	 ...
	 oldFile.renameTo(newFile); // 변경 전 파일(oldfile)을 새로운경로 및 이름을 가진(newfile)로 renameTo() 해준 것이다.
	 // 4. 다른 폼값 받기
 ...
 %>
 <!-- 파일을 업로드 할 때 파일명을 변경하는 주된 이유
 파일명 충돌 방지, 보안, 인코딩 문제 해결
 관리 용이성:
 파일을 업로드 할 때 일관된 명명 규칙을 사용하면, 나중에 파일을 관리하거나 찾을 때 편리
 업로드 시간을 파일명에 포함시키면, 언제 업로드 되었는지 쉽게 알 수 있습니다
 업로드한 파일명이 한글이라고 가정할 때 이를 브라우저에 표시하기 위해
 <img src='한글'.jpg'> 와 같이 써야한다.
 하지만 이 방식으로 웹 서버 인코딩 방식데 따라 한글이 깨질 수 있기 때문에
 영어나 숫자로 조합해주는 게 안전하다.
 파일 업로드용 작성폼 설정
 	method 속성은 반드시 post로, enctype 속성은 multipart/form-data로 지정
 	enctype은 폼값을 서버로 전송할 때의 인코딩 방식을 지정하는 속석
 	<input> 태그의 type 속성을 file로 지정
 ofile 	원본 파일명(original filename)
 sfile 		저장된 파일명(saved filename)
 파일의 확장자를 구하기 위해서 lastIndexof()메서드를 사용한 이유는? 
 . 이 2개 이상 포함할 수 있기 때문이다    2023년.가을하늘.jpg 방지키위해
  -->
